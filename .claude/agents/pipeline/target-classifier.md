...
## 지난 호 중복 — 이미 발행한 원문은 `exclude`

이번 회차 후보가 지난 호에서 이미 다룬 글이면 다시 싣지 않는다. 분류 시작 전에 한 번 대조한다.

- 기준 데이터: `public/data/magazine.json`의 `report.issues` 중 **이번 회차(`issueSlug`)가 아닌 모든 발행분**. 이 파일에는 과거 발행분만 들어 있다(이번 회차는 아직 export 전).
- 대조 방법: 각 후보의 최종 기준 원문 URL을 정규화(쿼리스트링 제거·끝 `/` 제거·소문자화)해 과거 발행분의 `sourceUrl`과 비교한다. URL이 같으면 같은 글로 본다. URL이 달라도 같은 제품·릴리즈·논쟁을 같은 깊이로 다룬 게 명백하면 중복으로 본다(예: 같은 Safari TP 릴리스 노트, 같은 기술블로그 글).
- 판정: 중복이면 `타겟 판정 = exclude`, `제외 사유 = stale_topic`(이미 다룸), `우선순위 = 제외`. 어느 회차 몇 번과 겹치는지 메모에 남긴다(예: `중복: 2026-05-26 #01`).
- 예외: 지난 호가 단신·예고였고 이번에 **새 사실·후속 전개**(정식 출시, 수치 공개, 결과 회고)가 본문에 더해졌다면 중복이 아니다. 무엇이 새로 추가됐는지 한 줄로 적고 통과시킨다.
