...
3. **본문 작성** — 카테고리별 형식 (2026-05-28 본문 간결화 기준: **인사이트 소제목 없음**, 요약 3~5불릿, 단락당 3문장 상한, 점검 질문 1개=한국 응용 통합, 모든 문장 경어체 `~입니다/~합니다`, 인용·관련 자료는 본문이 아니라 메타데이터 `관련 링크`):
   ...
   - DEV: `요약`(3~5불릿) + `매거진 인사이트` ((옵션)인용 → 구현 관점 단락 → 실무 적용 단락으로 끝, 점검 질문 없음)
   - 응용·점검 단락은 한국 클라이언트 1개를 박은 조건형 질문 1개("이미 했으면/도입 전이면" 양쪽에 걸리게, 독자가 이미 한다고 단정 금지).
   - **인용·관련 자료**(DEV 3종/SERVICE·DESIGN 2종)는 인사이트 본문에 넣지 말고 메타데이터 `- 요약:` 다음에 `- 관련 링크 N: [카테고리] 제목 | URL`로 둔다 → 출처 영역에 표시됨. URL 없는 근거 설명은 관련 링크로 만들지 않는다.
