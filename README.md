...
npm run collect:materials -- --no-fetch --no-upload
...
사이트와 뉴스레터에 노출되는 데이터는 항상 현재 Notion DB 기준입니다. 로컬/배포 모두 `NOTION_TOKEN`, `NOTION_DATABASE_ID`가 없으면 데이터를 만들지 않습니다. 사용자가 Notion에서 페이지를 삭제하거나 archive하면 다음 `fetch:notion`, `build`, 뉴스레터 발송 시점에 노출 대상에서도 빠집니다. 사이트는 Notion에 채택되어 있는 데이터를 별도 개수 제한 없이 읽고, 동일한 제목·브랜드·대카테고리 조합은 중복 노출하지 않습니다.
