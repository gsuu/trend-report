...
1. 수집 에이전트가 `data/platforms.json`의 국내 모니터링 범위, 공식 뉴스룸, 앱스토어, 공식 블로그, 주요 미디어, DEV 릴리즈 출처를 확인해 Service / Design / DEV 후보 20~30개를 `shortlist-20-30.md`로 모읍니다. 이 shortlist는 다시 4~7개로 줄이는 예비 목록이 아니라, 원문 검증 후 그대로 `magazine-report.md`에 작성할 기준 목록입니다.
