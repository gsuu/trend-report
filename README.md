...
이 매거진은 CTTD가 클라이언트와 서비스 개편, 캠페인, 디자인 시스템, 프론트엔드 구현 방향을 논의할 때 바로 참고할 수 있도록 씁니다. 1차 독자는 국내 이커머스·패션·뷰티·콘텐츠·플랫폼·핀테크 서비스를 운영하는 웹기획자, PM, 웹디자이너, UIUX 개발자입니다.

채택 기준은 “이 소식을 우리 서비스 회의에서 어떤 화면·정책·전환 흐름·디자인 기준·구현/QA 질문으로 바꿀 수 있는가”입니다. 이 질문으로 바뀌지 않는 일반 AI 앱 기능 묶음, 모델 성능 발표, 백엔드/API 운영, 클라우드 관리자 도구, OS 설정, 단순 프로모션은 유명하거나 최신이어도 메인 아티클에서 제외합니다. 상세 문체와 판정 기준은 [매거진 상세 글쓰기 가이드](docs/editorial-style-guide.md)의 `독자 타겟`을 따릅니다.
...
11. 아티클 태그: Notion과 매거진 화면에 노출되는 `태그`는 각 아티클의 핵심 단어 3개만 씁니다. 직무 태그를 반복하지 말고 기능명, 제품명, 설계 쟁점, 기술 쟁점 중심으로 고릅니다.
12. 제외 원칙: 단순 버그 수정, 단순 할인, 출처 없는 예측, 화면 변화 없는 홍보성 뉴스는 제외하거나 후보로만 둡니다.
13. 원문 역추적: 매거진, YouTube, 큐레이션 글에서 발견한 후보는 해당 글이 가리키는 원문을 찾아 `최종 기준 원문`으로 기록합니다.
14. 타겟 적합성 분류: shortlist 전에 후보를 `core_ecommerce / commerce_adjacent / design_dev_reference / weak_promo / exclude`로 나눕니다. `weak_promo`와 `exclude`는 발행 후보로 쓰지 않습니다.
15. 이커머스 우선: 최종 발행 후보 20~30개는 품질 기준으로 고르되, 같은 품질이면 상품 탐색, 비교, 장바구니, 결제, 멤버십, 리뷰, 추천, 재구매, CRM 흐름이 보이는 이커머스 후보를 먼저 둡니다.
16. 균형 수집: 수집 단계에서는 Service / Design / DEV와 공식 출처, 국내 매거진, 공식 YouTube, AI·디자인 도구, DEV 릴리즈를 모두 확인합니다.
17. 후보군 글 등록 제한: 후보 발견 출처가 매거진/개인 블로그/YouTube일 경우, 최종 기준 원문이 대기업·주요 스타트업·공식 기관 자료로 확인될 때만 글 등록합니다.

## 목표 수집 수준

샘플 PDF 수준의 원천 데이터를 기준으로 삼습니다. 즉, 한 이슈는 단순 기사 요약이 아니라 아래 정보가 있어야 채택합니다.

매주 발행할 채택 이슈는 Service / Design / DEV를 합쳐 보통 20~30개 사이에서 고릅니다. 숫자는 고정 상한이 아니라 원문 품질과 독자 적합성 기준이며, 사이트와 뉴스레터는 `magazine-report.md`에 작성된 이슈를 별도 개수 제한 없이 읽습니다.
...
1. UIUX `서비스 변화 요약`: 업데이트, 서비스 맥락, 변경 전/후, 수치·팩트를 한 섹션 안에 묶습니다. `업데이트`는 제목 반복이 아니라 원문의 기능명, 출시 범위, 달라진 화면이나 정책이 보이도록 2줄 정도로 씁니다.
2. DEV `기술 변화 요약`: 업데이트, 기술 맥락, 변경 전/후, 확인할 영향을 한 섹션 안에 묶습니다. `업데이트`는 기술명, 버전, 릴리즈 범위, 핵심 변화가 보이도록 2줄 정도로 씁니다.
3. 매거진 인사이트: 원문 핵심 요약을 먼저 담고, Service/Design은 `왜 지금 이 업데이트인가`, `사용자는 무엇을 덜 해도 될까`, `설계 관점`을 각각 3줄 정도로 확장합니다. DEV는 `왜 지금 이 업데이트인가`와 `구현 관점`을 각각 3줄 정도로 확장합니다.
...
웹사이트 상세페이지에만 길게 보여줄 내용은 `매거진 인사이트` 섹션에 작성합니다. 이 섹션은 뉴스레터 목록에는 노출하지 않고, 매거진 아티클 페이지에서만 렌더링합니다. 여기에는 원문에서 확인한 핵심 사실을 요약한 뒤 `배경`, `사용자 여정 해석`, `클라이언트 적용 포인트`를 하나의 매거진 글처럼 씁니다. 내부 선별 방식이나 검수 방법론은 본문에 쓰지 않습니다. 문체는 [매거진 상세 글쓰기 가이드](docs/editorial-style-guide.md)를 따릅니다.
...
1. ecommerce: 종합몰, 마켓플레이스, 홈쇼핑, 배달/생활 커머스
2. fashion: 패션 플랫폼, 브랜드관, 룩북, 찜/재입고, 스타일 추천
3. beauty: 뷰티 커머스, 리뷰/성분/피부 진단, 개인화 추천
4. department_store: 백화점 앱, VIP/멤버십, 온라인 혜택, 선물/픽업
5. platform: 로컬, 커뮤니티, 콘텐츠, 리서치, 검색/추천처럼 공급자와 사용자를 연결하는 플랫폼 서비스
6. fintech: 결제, 송금, 정산, 포인트, 캐시, 가맹점/사장님 도구처럼 돈의 흐름을 다루는 서비스
7. book_content: 도서, 전자책, 웹툰/웹소설, 이어보기, 콘텐츠 추천

수집은 국내 모니터링 범위의 모든 서비스를 확인합니다. 위 순서는 수집 제외 기준이 아니라, 리포트에 올릴 이슈를 선별하고 배치할 때의 우선순위입니다. shortlist를 추릴 때는 이커머스 core 후보를 먼저 채우고, platform·fintech·global service 후보는 이커머스 고객 여정에 바로 대입할 수 있을 때만 보조로 둡니다.
...
3. beauty: 뷰티 커머스, 리뷰/성분/피부 진단, 개인화 추천
4. department_store: 백화점 앱, VIP/멤버십, 온라인 혜택, 선물/픽업
5. platform: 로컬, 커뮤니티, 콘텐츠, 리서치, 검색/추천, 양면시장 플랫폼
6. fintech: 결제, 송금, 정산, 포인트, 캐시, 금융/가맹점 운영 UX
7. book_content: 도서, 전자책, 웹툰/웹소설, 이어보기, 콘텐츠 추천
...
1. `기술 변화 요약`: 업데이트, 기술 맥락, 변경 전/후, 확인할 영향을 씁니다. `업데이트`는 원문을 보지 않아도 무엇이 발표됐는지 알 수 있게 2줄 정도로 씁니다.
2. `왜 지금 이 업데이트인가`: 릴리즈 배경과 기술 흐름을 3줄 정도로 설명합니다. 원문에 나온 기술명, 버전, 기능 범위, 적용 조건을 포함합니다.
3. `구현 관점`: 프레임워크, 브라우저, 빌드/품질 도구, 런타임, 배포 환경 관점에서 봐야 할 판단을 3줄 정도로 씁니다.
...
주간 작업은 [매거진 에이전트 운영 흐름](docs/magazine-agent-workflow.md)의 `수집 에이전트 → 원문 검증 에이전트 → 타겟 적합성 분류 에이전트 → shortlist 작성 에이전트 → 카테고리 분류 에이전트 → 글쓰기 에이전트 → 리뷰 에이전트 → QA 에이전트` 순서를 따릅니다.

1. 수집 에이전트가 `data/platforms.json`의 국내 모니터링 범위, 공식 뉴스룸, 앱스토어, 공식 블로그, 주요 미디어, DEV 릴리즈 출처를 확인해 Service / Design / DEV 후보와 증거를 모읍니다. 단, 제휴카드, 쿠폰/e쿠폰, 콘텐츠 제휴, 외부 AI 연동, 멤버십 프로모션처럼 화면·플로우 증거가 약한 소식은 후보 단계에서 제외합니다.
...
3. 원문 검증 에이전트가 후보 발견 출처와 최종 기준 원문을 분리합니다. 원문에서 구체 사실 3개 이상을 뽑지 못하면 해당 shortlist 항목은 `수집했지만 제외한 것`으로 이동합니다.
4. 타겟 적합성 분류 에이전트가 후보를 `core_ecommerce / commerce_adjacent / design_dev_reference / weak_promo / exclude`로 나누고, `weak_promo`와 `exclude`는 제외 메모로 보냅니다.
5. Shortlist 작성 에이전트가 이커머스 core 후보를 최우선으로 20~30개를 `shortlist-20-30.md`에 정리합니다. 이 shortlist는 다시 4~7개로 줄이는 예비 목록이 아니라, 원문 검증 후 그대로 `magazine-report.md`에 작성할 기준 목록입니다.
6. 카테고리 분류 에이전트가 Service / Design / DEV와 소분류를 확정합니다. 디자인 AI는 `design/ai`, 개발 AI는 `dev/ai`로 정리하고, 화면 구현과 무관한 DATA/API 중심 글은 제외합니다.
7. Service / Design / DEV와 출처 유형이 한쪽으로 몰리지 않았는지 확인합니다. 채택할 이슈가 없는 축도 확인한 출처와 제외 사유를 남깁니다.
8. 이미지/미디어 검수 에이전트가 원천 수집 파일의 이미지 출처를 확인하고, 필요하면 `scripts/tracking/fill_missing_images.py collections/YYYY-MM-DD-raw-trend-collection.md`로 빈 이미지를 보강합니다.
9. `scripts/legacy/new_report.py`로 이번 주 리포트 파일을 생성합니다.
10. 글쓰기 에이전트가 발행 리포트에 UIUX 이슈는 `서비스 변화 요약`, `매거진 인사이트` 순서로, DEV 이슈는 `기술 변화 요약`, `매거진 인사이트` 순서로 작성합니다.
11. 글쓰기 에이전트는 각 글에 원문에서 확인한 구체 항목 3개 이상을 반영하고, 최종 리포트 하단의 `수집했지만 제외한 것`에 제외 후보의 출처, 대분류, 제외 사유, 다시 볼 조건을 짧게 남깁니다.
12. 리뷰 에이전트가 작성된 20~30개 매거진 문구의 `출처 URL`을 다시 열어 원문 대조 리뷰를 수행합니다. 원문 핵심이 빠졌거나 일반론으로 바뀐 글, 카테고리가 잘못된 글, 원문에 없는 효과를 단정한 글은 원문 기준으로 다시 수정합니다. `혜택 조건을 쉽게`, `결제 후 다음 거래`, `추천을 안전하게 연결`처럼 어느 서비스에도 붙는 문장만 남는 글은 수정하지 말고 제외합니다.
13. 카테고리/라우팅 QA 에이전트가 `dev/ai_design`, `dev/ai_dev`, `dev/data_api` 같은 잘못된 경로와 article 번호/route를 점검합니다.
14. Notion 동기화 에이전트가 `runs/YYYY-MM-DD/magazine-report.md`, `public/data/magazine.json`, Notion DB의 제목, 카테고리, deck, 태그가 같은지 확인하고 필요한 곳을 다시 맞춥니다.
15. 뉴스레터 프리뷰 에이전트가 이메일 카드 길이, 링크, 이미지, CTA를 확인합니다. 테스트 확인 전에는 최종 수신자에게 발송하지 않습니다.
16. UIUX `서비스 변화 요약`에는 업데이트, 서비스 맥락, 변경 전/후, 실제 수치만 묶어 넣고, DEV `기술 변화 요약`에는 업데이트, 기술 맥락, 변경 전/후, 확인할 영향을 묶어 넣습니다. 요약 라벨은 유지하되 `업데이트`는 2줄 정도로 풉니다. Service/Design의 `왜 지금 이 업데이트인가`, `사용자는 무엇을 덜 해도 될까`, `설계 관점`과 DEV의 `왜 지금 이 업데이트인가`, `구현 관점`은 각각 3줄 정도로 써서 원문을 읽은 밀도가 느껴지게 합니다.
...
NEWSLETTER_UNSUBSCRIBE_SECRETS=optional-previous-secret,optional-local-secret
...
구독자 DB는 `Email`, `Status`, `Audience`, `Unsubscribed At` 속성을 사용합니다. `Status`가 `Unsubscribed` 또는 `Inactive`인 행은 발송 대상에서 제외됩니다. `Audience`가 비어 있으면 전체 뉴스레터 대상으로 보고, 값이 있으면 `Service`, `Design`, `DEV`에 맞는 구독자에게만 발송합니다. 기존 `Service/Design` 값은 Service와 Design을 모두 구독한 것으로 처리합니다. 메일 하단의 구독 해지 링크는 `/api/unsubscribe`에서 확인 후 `Status=Unsubscribed`, `Unsubscribed At=현재 시각`으로 갱신합니다. `NEWSLETTER_UNSUBSCRIBE_SECRET`은 새 링크 생성에 쓰는 현재 secret이고, `NEWSLETTER_UNSUBSCRIBE_SECRETS`는 이미 발송한 링크를 살리기 위한 이전 secret 목록입니다. 로컬 스크립트로 발송할 때는 Production API의 현재 또는 fallback secret과 반드시 같은 값을 사용합니다.
