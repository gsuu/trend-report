...
14. 균형 수집: 최종 best 20은 품질 기준으로 고르되, 수집 단계에서는 Service / Design / DEV와 공식 출처, 국내 매거진, 공식 YouTube, AI·디자인 도구, DEV 릴리즈를 모두 확인합니다.
15. 후보군 글 등록 제한: 후보 발견 출처가 매거진/개인 블로그/YouTube일 경우, 최종 기준 원문이 대기업·주요 스타트업·공식 기관 자료로 확인될 때만 글 등록합니다.
...
1. 수집 에이전트가 `data/platforms.json`의 국내 모니터링 범위, 공식 뉴스룸, 앱스토어, 공식 블로그, 주요 미디어, DEV 릴리즈 출처를 확인해 Service / Design / DEV 후보 20~30개를 `shortlist-20-30.md`로 모읍니다. 이 shortlist는 다시 4~7개로 줄이는 예비 목록이 아니라, 원문 검증 후 그대로 `magazine-report.md`에 작성할 기준 목록입니다. 단, 제휴카드, 쿠폰/e쿠폰, 콘텐츠 제휴, 외부 AI 연동, 멤버십 프로모션처럼 화면·플로우 증거가 약한 소식은 후보 단계에서 제외합니다.
...
10. 리뷰 에이전트가 작성된 20~30개 매거진 문구의 `출처 URL`을 다시 열어 원문 대조 리뷰를 수행합니다. 원문 핵심이 빠졌거나 일반론으로 바뀐 글, 카테고리가 잘못된 글, 원문에 없는 효과를 단정한 글은 원문 기준으로 다시 수정합니다. `혜택 조건을 쉽게`, `결제 후 다음 거래`, `추천을 안전하게 연결`처럼 어느 서비스에도 붙는 문장만 남는 글은 수정하지 말고 제외합니다.
