...
1. 서비스 변화 요약: 업데이트, 서비스 맥락, 변경 전/후, 수치·팩트를 한 섹션 안에 묶습니다.
2. 매거진 인사이트: 뉴스 요약을 반복하지 않고 배경, 사용자 여정 해석, 클라이언트 질문을 하나의 글로 확장합니다.
...
8. 발행 리포트에는 각 이슈를 `서비스 변화 요약`, `매거진 인사이트` 순서로 작성합니다.
...
10. `서비스 변화 요약`에는 업데이트, 서비스 맥락, 변경 전/후, 실제 수치만 묶어 넣고, `매거진 인사이트`에는 배경, 사용자 여정 해석, 클라이언트 적용 포인트를 하나의 글로 추가합니다.
