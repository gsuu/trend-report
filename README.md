...
제목은 발표 사실을 그대로 적기보다 각 타깃이 확인해야 할 가장 중요한 판단 포인트로 씁니다. 예를 들어 `지그재그가 임부복 카테고리를 신설했습니다`보다 `상황 언어가 온라인 카테고리 구조로 바뀌고 있는가`처럼 웹기획자, PM, 웹디자이너, UIUX 개발자가 바로 확인할 질문이 보여야 합니다. 설명에는 발표 사실, 날짜, 범위, 기능명을 넣어 근거를 보완합니다.
