...
6. 크로스보더 가치: 해외 진출, 현지 법인, 오프라인 매장, 역직구 성장처럼 국내 서비스 전략에 영향을 주는 이슈는 UI 화면 변화가 없어도 후보로 수집합니다.
7. 제외 원칙: 단순 버그 수정, 단순 할인, 출처 없는 예측, 화면 변화 없는 홍보성 뉴스는 제외하거나 후보로만 둡니다.
...
7. 이미지 근거: 화면 변화가 중요한 이슈는 공식 스크린샷, 앱스토어 이미지, 공식 뉴스룸 이미지처럼 출처가 확인되는 이미지를 함께 남깁니다.
...
4. 해외 진출, 현지 법인, 오프라인 매장, 역직구 성장처럼 서비스 확장 전략을 보여줍니다.
...
4. 로그인, 뷰어, 책갈피, 재고 확인, 매장 찾기 등 사용성 개선
5. 단순 안정화는 제외
...
- 공식 앱스토어/플레이스토어 스크린샷, 공식 뉴스룸 이미지, 공식 이벤트 페이지 이미지를 우선합니다.
...
- 새로 추가한 후보/아티클은 발행 전에 `scripts/fill_missing_images.py`로 빈 `이미지:`를 먼저 채웁니다. 자동 탐색은 App Store 공식 스크린샷, 페이지 `og:image`, `twitter:image` 순으로 시도합니다. 그래도 없으면 `이미지 확보 메모:`에 못 찾은 이유를 남깁니다.

## 국내 모니터링 범위

- 1순위 패션: 무신사, SSF SHOP, 코오롱몰, LF몰, W Concept, 29CM, EQL 등
- 2순위 이커머스: 쿠팡, SSG.COM, 롯데ON, G마켓, 네이버플러스 스토어, 당근마켓 등
...
3. 후보 이슈를 먼저 `templates/raw-collection.md` 형식으로 풍부하게 수집합니다. 이때 기능명만 적지 않고 `출처 원문`, `화면·플로우 관찰`, `사용자 행동 변화`, `수치·팩트`, `비즈니스 맥락`, `매거진 인사이트 확장 메모`까지 채웁니다.
4. 수집 카드마다 `미팅 활용도`, `검증 상태`, `채택 여부`를 표시합니다.
5. 채택 이슈를 `UI/UX·서비스 업데이트`, `신규 서비스·UX 실험`, `시장·전략`으로 나눕니다.
6. `scripts/fill_missing_images.py collections/YYYY-MM-DD-raw-trend-collection.md`로 원천 수집 파일의 빈 이미지를 먼저 보강합니다.
7. `scripts/new_report.py`로 이번 주 리포트 파일을 생성합니다.
8. 발행 리포트에는 UIUX 이슈를 `서비스 변화 요약`, `매거진 인사이트` 순서로, DEV 이슈를 `기술 변화 요약`, `매거진 인사이트` 순서로 작성합니다.
9. `scripts/fill_missing_images.py reports/YYYY-MM-DD-uiux-web-service-weekly-trend-report.md`를 실행해 새 아티클 이미지 누락을 한 번 더 막습니다.
10. UIUX `서비스 변화 요약`에는 업데이트, 서비스 맥락, 변경 전/후, 실제 수치만 묶어 넣고, DEV `기술 변화 요약`에는 업데이트, 기술 맥락, 변경 전/후, 확인할 영향을 묶어 넣습니다. `매거진 인사이트`에는 배경, 사용자 여정 해석 또는 기술 적용 포인트를 하나의 글로 추가합니다.
...
Cron 함수는 KST 기준 지난주 월요일 00:00부터 이번 주 월요일 00:00 전까지의 Notion 데이터만 읽습니다. 이후 `대카테고리=DEV` 이슈와 나머지 UIUX 이슈를 나눈 뒤 각각 기존 뉴스레터 톤의 HTML 메일로 발송합니다.
