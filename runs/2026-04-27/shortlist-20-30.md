...
전체 수집 214개 중 CTTD 매거진 상세 글로 작성할 기준 후보를 20~30개 범위에서 추립니다. 이 파일은 다시 4~7개로 줄이는 예비 목록이 아니라, 원문 검증 후 그대로 `magazine-report.md`에 작성할 기준 목록입니다.

## 선정 기준

- CTTD 포트폴리오와 가까운 패션, 뷰티, 라이프스타일, 이커머스, 로컬/커머스 플랫폼, 디자인 시스템, 프론트엔드 QA 이슈를 우선했습니다.
- 온라인 화면, 기능, 사용자 플로우, 추천/리뷰/결제/저장/재방문 변화가 확인되는 항목만 Service 후보로 남겼습니다.
- 단순 캠페인, 할인, 카드 제휴, 콘텐츠 홍보, 모델/화보, 매장 오픈, 채용/실적/수상 소식은 제외 우선으로 봤습니다.
- 뉴스레터, 매거진, 팟캐스트 글은 최종 원문이 아니라 후보 발견 출처로만 봅니다.
- 같은 브랜드에서 여러 후보가 들어온 경우 독자 가치가 큰 항목 최대 2개만 남겼습니다.
...
- 직무 태그: 웹서비스기획, 웹디자인
- 채택 사유: Gemini 앱 업데이트 중 대화 지속, 생성/검색 보조, 개인화 경험처럼 사용자 인터페이스에 닿는 항목이 있으면 AI 앱 UX 후보로 볼 수 있습니다.
- 확인 필요: 단순 기능 모음이면 제외하고, 화면 흐름이 바뀐 항목만 골라야 합니다.
...
- 직무 태그: 웹DEV
- 채택 사유: JS 생태계 이슈 중 UI 구현, 빌드, 테스트, 미디어/그래픽 UI와 연결되는 원문 링크를 추려 쓰기 좋습니다.
- 확인 필요: 뉴스레터 전체를 그대로 요약하지 않고, 실제 원문 링크 기준으로 브라우저/빌드/프레임워크 영향이 있는 항목만 반영합니다.
...
- DesignDB 기술 트렌드와 해외 디자인뉴스 다수는 정책·전시·시장 리포트 성격이라 CTTD UIUX 상세 글 근거로는 약합니다.
- Into Design Systems의 컨퍼런스, 해커톤, 강의/녹화 판매 성격 글은 광고성이 강하면 제외하고, 공식 원문으로 역추적되는 글만 보조 출처로 남깁니다.
- OpenAI GPT-5.5, 시스템 카드, 임상/프라이버시 글은 중요도와 별개로 이번 CTTD 웹서비스·디자인·DEV 실무 타겟과 거리가 있어 보류합니다.
- Google Gemini Embedding, Deep Research, Google One AI Studio 구독 글은 화면 UX보다 모델/개발자 도구 성격이 강해 보류합니다.
- Chrome WebGPU, Structured Clone for extension messaging는 UIUX 개발 실무와 직접 연결되는 후보가 있으면 보조로만 둡니다.
- 같은 브랜드 후보는 shortlist 구성 단계에서 최대 2개 원칙을 유지합니다.
