...
#### 08. [AccessLint] AccessLint MCP로 브라우저에서 접근성 위반을 먼저 찾고, AI 에이전트는 실제 수정에 집중하게 하는 워크플로우
...
- 업데이트: AccessLint MCP와 브라우저 MCP를 함께 사용해 live URL 또는 localhost에서 렌더링된 DOM을 접근성 감사 대상으로 삼는 워크플로우가 소개됐습니다.
- 기술 맥락: 전체 컴포넌트 파일을 에이전트가 읽고 판단하는 방식은 저장소 크기만큼 LLM 비용이 커지지만, 접근성 rule check 자체는 브라우저에서 실행되는 결정적 JavaScript로 처리할 수 있습니다.
- 작동 방식: 에이전트가 AccessLint의 audit_browser_script를 받고, 브라우저 MCP가 해당 스크립트를 페이지에서 실행한 뒤 ruleId, selector, html, impact, message가 담긴 JSON violation 목록을 돌려줍니다.
- 변경 후: 에이전트는 모든 파일을 훑지 않고 violation에 포함된 data-testid, aria-label, visible text, class name 같은 단서를 grep해 관련 소스 파일만 열고 수정합니다.
- 확인할 영향: 접근성 자동화의 비용 기준이 “코드베이스 전체 크기”에서 “실제로 깨진 화면 요소 수”로 이동하며, nightly scan이나 PR용 접근성 수정 봇을 현실적으로 운영할 수 있습니다.

##### 매거진 인사이트

> 브라우저는 문제를 찾고, 에이전트는 고친다는 역할 분리가 이 글의 핵심입니다.

###### 왜 지금 볼만한가

AI 코드 에이전트를 접근성 QA에 붙일 때 가장 쉬운 접근은 저장소 전체를 읽히는 것입니다. 하지만 원문은 이 방식이 작은 데모에서는 좋아 보여도 실제 제품에서는 비용과 범위가 빠르게 커진다고 지적합니다.
...
MCP 흐름은 구체적입니다. AccessLint MCP가 감사용 스크립트를 제공하고, Playwright 또는 Chrome DevTools MCP가 현재 페이지에서 이를 실행합니다. 이후 audit_browser_collect 결과로 나온 selector와 HTML 단서를 바탕으로 에이전트가 소스 파일을 찾아 alt text, label, ARIA 값, contrast, 구조 문제를 수정합니다.
...
- 운영 중인 URL이나 preview deploy URL에 nightly 접근성 스캔을 걸고, 변경이 있을 때만 하나의 rolling PR을 갱신하는 방식으로 시작할 수 있습니다.
- 로컬 개발 중에는 localhost 페이지를 대상으로 같은 루프를 돌려, JSX만 봐서는 알 수 없는 포커스 상태, ARIA 상태, 대비 문제를 확인합니다.
- 자동 수정 PR은 reviewer가 overreach를 잡는 전제로 운영하고, 문구 판단이 필요한 alt text나 label은 제품 맥락 검토를 남겨둡니다.
