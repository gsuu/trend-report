...
3. **원문 검증 없이 글쓰기 금지 + 본문은 사실 기반만** — 모든 매거진 글은 최종 기준 원문(공식 뉴스룸/릴리즈 노트/제품 블로그)을 직접 열고 작성합니다. RSS 발췌나 큐레이션 글에서 본문을 만들지 않습니다. 또한 본문 모든 문장은 **공개·관찰·인용된 사실** 셋 중 하나에 묶여야 하며, **추측·해석·전망 표현은 본문 금지**입니다. 추측은 인용 박스(`> 인용: "원문" — 발화자 · 매체`)로 분리해 "발화자가 그렇게 말했다"는 사실로 변환합니다. 작가/편집자 시점은 `📝 큐레이션 노트` 한 자리에만 분리. 자세한 가이드는 [v4 사실 기반 회의록](docs/article-content-redesign-v4-facts-only-2026-05.md).
