...
1. **뉴스레터 발송 순서** — 테스트 메일은 `jisuk@cttd.co.kr`로만 먼저 보내고, 사용자가 채팅에서 명시 확인한 뒤에야 `cxd@cttd.co.kr`로 최종 발송합니다. 이 순서는 사용자가 한 번 승인했더라도 매 발송마다 다시 확인합니다. 자세한 흐름은 `AGENTS.md`의 Newsletter Approval Flow.
