...
## About CTTD (모든 가정의 출발점)

**CTTD는 디지털 에이전시다.** 자체 서비스를 만들지 않고 여러 클라이언트사(패션·뷰티·식품·금융·콘텐츠·플랫폼 등)의 프로젝트를 수주받아 디지털 산출물을 제작·운영한다.

따라서 본 매거진의 모든 글은 **"우리 서비스에 적용"** 자료가 아니라 **"CTTD 직원이 클라이언트 회의·시안·구현에서 인용·제안"** 하는 자료다. 작성·검수·라벨링·메타 어조는 모두 이 시점을 기준으로 한다.

본문은 항상 다음 흐름으로 수렴한다 (v3·v3.1·v4 합의):

1. **매거진 블록 (사실 기반 본문, 씬→흐름→함의 600-900자)** — 외부 브랜드·서비스에서 무엇이 일어났는가. 모든 문장은 공개·관찰·인용된 사실 셋 중 하나에 묶인다. 추측·해석·전망은 본문 금지.
2. **🗂️ 큐레이션 인덱스 3블록** — 📝 큐레이션 노트(왜 이 글을 골랐나, 편집자 한 시점) · 📖 이 글에서 만나는 것(사람·장면·숫자·도구) · 📚 같이 살펴보면 좋은 글(누적+외부)
3. **📚 심화 섹션 (선택)** — H3 단위 1-3개, 라이터 의지로 디테일 보존

작가/편집자 시점은 **`📝 큐레이션 노트` 한 자리에만 분리**한다. 본문에서 작가가 직접 추측·해석·평가하는 문장은 금지하고, 모든 추측은 인용 박스(`> 인용: "원문" — 발화자 · 매체`)로 분리해 사실로 변환한다.
...
| areaKey | 카테고리 라벨 | CTTD 내부 직군 | **사용처 (외부)** | 한 줄 정의 | 제외 범위 |
|---|---|---|---|---|---|
| `service` | Service | 기획자 (PM·전략 담당) | **클라이언트 의사결정자(CEO/PO/사업 리드) 미팅에서 제안 자료** | 이커머스·서비스 구조·전환·정책·플로우·시장 전략을 보는 독자 | — |
| `design` | Design | 디자이너 (UIUX·비주얼·브랜드) | **클라이언트 브랜드·UX 시안 작업에 인용** | UI·비주얼·브랜드·콘텐츠 표현·디자인 시스템을 보는 독자 | — |
| `dev` | DEV | 퍼블리셔 (UIUX 퍼블리셔) | **클라이언트 프로젝트 구현·환경설정에 적용** | 마크업·CSS·접근성·인터랙션 구현 + Vite·11ty·Astro 같은 정적 사이트 빌드·환경설정 | 데이터·백엔드·서버·DB·인프라·ML |

운영 원칙:

- 한 글은 정확히 하나의 `areaKey`를 가진다. 예: SERVICE 글이 디자이너에게도 유익한 경우는 본문에서 다루고, 글의 1차 독자(우리 직원의 어느 직군)만으로 카테고리를 정한다.
- 사이트 헤더·구독 모달·태그·문서에서 "Service/Design/DEV" 카테고리 라벨과 함께 직군 라벨(기획자/디자이너/퍼블리셔)을 병기한다. 보조 문구는 **사용처(클라이언트)** 시점으로 쓴다.
...
- `meeting_question`은 "**우리 에이전시가 클라이언트 회의에서 제안 직후 던질 한 문장 질문**". `핵심 인용`은 "**클라이언트 제안 슬라이드에 그대로 옮길 한 문장**". `출처 검증` 박스는 "**클라이언트 임원의 깐깐한 출처 질문에 즉답하기 위한 박스**".
