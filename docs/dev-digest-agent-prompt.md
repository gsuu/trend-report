...
`매거진 인사이트` 소제목은 **3개 잠금 셋(2026-05-24 회의 결정으로 4→3 압축)** 외 추가하지 않습니다: `이 변화가 노리는 것` / `구현 시 짚을 점` / `점검 질문`. 직전 4개 잠금셋(`왜 지금 / 구현 관점 / 실무에 어떻게 / 같이 보면 좋은 기술`)으로 작성된 누적 글은 그대로 두고 신규 글부터 새 표준을 따릅니다. `같이 보면 좋은 기술`은 별도 슬롯이 아니라 `구현 시 짚을 점` 마지막 단락에 1-2줄로 흡수합니다. 각 소제목 바로 아래에는 회색 보조 한 줄(prompt-hint, `>` blockquote)이 의무입니다 — 자세한 형식은 [매거진 원고 출력 기준 — 잠금 소제목 셋](magazine-writing-standard.md#잠금-소제목-셋-locked-sub-headings).
