...
`매거진 인사이트`는 **소제목을 쓰지 않습니다**(2026-05-28 본문 간결화 기준). (옵션) 인용 1문장 → 구현 관점 단락(한국 4축 중 1개 한 문장 포함) → 실무 적용 단락으로 끝냅니다. 단락당 3문장 상한. DEV는 **점검 질문을 두지 않습니다.** 인용 3종(`[합성 데모]`/`[공식 사양]`/`[프로덕션 사례]`)은 본문이 아니라 메타데이터 `- 관련 링크 1~3`으로 둬 출처 영역에 표시합니다. 옛 잠금 소제목(`왜 지금 이 업데이트인가` / `구현 관점` / `실무에 어떻게 적용할 수 있을까` / `같이 보면 좋은 기술`)은 단락 흐름에 흡수합니다. 모든 문장은 경어체(`~입니다/~합니다`)로 씁니다.

요약은 자체 완결되어야 합니다. 메타 `요약:`과 본문 `##### 요약`의 첫 불릿(글 전체 결론 한 줄)만 읽어도 무엇이 어디에 추가/변경됐는지 그려져야 합니다. 또한 첫 두 불릿 안에는 기술명·도구명·API/속성·버전·브라우저 조건 중 하나 이상이 반드시 들어갑니다. `~를 소개합니다`, `~를 다룹니다`로 시작하는 표지 문장은 금지합니다. 본문 요약 불릿은 **3~5개, 불릿당 2문장 상한**입니다(채움 불릿 금지).
